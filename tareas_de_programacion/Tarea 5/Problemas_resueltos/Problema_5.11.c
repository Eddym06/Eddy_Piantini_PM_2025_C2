...
scanf(“%d”, &TAM); }
while (TAM>MAX 􏰁 TAM<1); /* Se verifica que el tamaño del arreglo sea ➥correcto. */
Lectura(VEC, TAM);
...
printf(“\nEl elemento se encuentra en la posición: %d”, RES); else
...
{
int ELE, IZQ = 0, CEN, DER = T-1, BAN = 0; while ((IZQ <= DER) && (!BAN))
{
