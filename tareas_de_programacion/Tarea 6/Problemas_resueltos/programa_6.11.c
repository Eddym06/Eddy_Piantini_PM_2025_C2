...
void F1(float [][12], int, int, float *, float *); void F2(float [][12], int, int);
void F3(float *, int);
...
LecturaM(FON, 5, 12); LecturaV(PRE, 5);
F1(FON, 5, 12, PRE, REN); F2(FON, 5, 12);
F3(REN, 5); }
...
for (I=0; I<F; I++)
for (J=0; J<C; J++) {
printf("Precio fondo %d\t mes %d: ", I+1, J+1); scanf("%f", &A[I][J]);
} }
...
for (I=0; I<T; I++)
{
printf("Precio Fondo %d al 31/12/2003: ", I+1); scanf("%f", &A[I]);
} }
...
int I, J;
float SUM, PRO;
printf("\n\nPROMEDIO ANUALIZADO DE LAS ACCIONES DE LOS FONDOS"); for(I=0; I<F; I++)
{
SUM = 0;
for(J=0; J<C; J++)
SUM += A[I][J]; PRO = SUM / C;
