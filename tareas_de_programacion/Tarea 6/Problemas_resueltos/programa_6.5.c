...
const int MAX = 50; const int EXA = 4;
void Lectura(float [MAX][EXA], int); void Funcion1(float [MAX][EXA], int); void Funcion2(float [MAX][EXA], int);
void main(void) {
int NAL;
...
scanf("%d", &NAL);
/* Se verifica que el número de alumnos del grupo sea válido. */ (NAL > MAX || NAL < 1);
Lectura(ALU, NAL); Funcion1(ALU, NAL); Funcion2(ALU, NAL); }
