...
for (i=0; i<n; i++)
{
printf("Ingrese la línea %d de texto. Máximo 50 caracteres: ", i+1); fflush(stdin);
gets(FRA[i]);
}
printf("\n");
for (i=0; i<n; i++) {
/* Se lee la cadena de caracteres dentro del ciclo. */
strcpy(cad, FRA[i]); t = longitud (cad); if (t > l)
{
l = t;
p = i; }
}
printf("\nLa cadena con mayor longitud es: "); puts(FRA[p]);
printf("\nLongitud: %d", l);
}
